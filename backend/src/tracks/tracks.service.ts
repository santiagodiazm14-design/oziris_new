...
export class TracksService {
  private inMemoryTracks: Track[] = [...INITIAL_TRACKS];

  constructor(private readonly prisma: PrismaService) {}

  async findAll(genre?: string): Promise<Track[]> {
    try {
      const where = genre ? { genre: { equals: genre, mode: 'insensitive' as const } } : {};
    ...
    if (genre) {
      return this.inMemoryTracks.filter(
  ...
  async findOne(id: string): Promise<Track> {
    ...
    if (!track) {
      throw new NotFoundException(`Beat con ID ${id} no encontrado.`);
    }
    return track;
  }
  ...
  private formatTrack(t: any): Track {
    return {
      id: t.id,
      title: t.title,
